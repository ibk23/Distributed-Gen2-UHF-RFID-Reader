...
class reader_top_block(gr.top_block):
  ...
  def __init__(self):
    ...
    self.adc_rate = 0.1e6            # ADC rate (2MS/s complex samples)
    ...
    self.tx_gain_1   = 9                    # RFX900 no Tx gain option
    self.tx_gain_2   = 9                    # RFX900 no Tx gain option
    ...
    if (DEBUG == False) : # Real Time Execution
      ...
      self.connect(self.file_source,  self.low_pass)
