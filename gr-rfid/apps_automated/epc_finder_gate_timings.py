...
2 * half_symbol_length * [1], half_symbol_length * [-1], half_symbol_length * [1],
    ...
    filtered_peak_locations = filtered_peak_locations[correlated[filtered_peak_locations] > 0.85*np.amax(correlated)]
...
def count_rn16s_gate_timings(numpyarray):
    percentage_between = 50  # Should be 50, though lower value may give better results
    # Perform a weighted mean between the maximum and minimum values.
    # Due to clipping, 50% occurs quite high up, causing some false transitions.
    avg = np.average([np.amin(numpyarray), np.amax(numpyarray)], weights=[100 - percentage_between, percentage_between])
    ...
    xdiff = np.diff(zc)
    print(xdiff)
    sampled_signal = [1 * half_symbol_length , 2 * half_symbol_length , half_symbol_length , half_symbol_length ,
                                     2 * half_symbol_length , half_symbol_length,
                                     3 * half_symbol_length , half_symbol_length]
...
def count():
    ...
    count_rn16 = count_rn16s_gate_timings(abs_f)
